    ...
    <div className="w-[280px] bg-background flex flex-col h-full border-r border-white/10 text-sm">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1.5">
        ...
        {['System Architecture Design', 'Ollama UI Setup'].map((chat, i) => (
          <button key={i} className={`w-full text-left p-3 rounded-2xl truncate transition-colors ${i === 0 ? 'bg-surfaceElevated text-white' : 'text-gray-300 hover:bg-surface'}`}>
            {chat}
          </button>
        ))}
        ...
        {['React Window Management', 'Tailwind CSS Downgrade', 'Vite Configuration Error'].map((chat, i) => (
          <button key={i} className="w-full text-left p-3 rounded-2xl truncate text-gray-300 hover:bg-surface transition-colors">
            {chat}
          </button>
        ))}
      </div>
      ...
      <div className="p-4 border-t border-white/10 space-y-1.5">
        <button className="flex items-center gap-3 w-full hover:bg-surfaceElevated transition-colors rounded-2xl p-3 text-left text-gray-300">
          <LayoutGrid size={18} />
          Models
        </button>
        <button 
          onClick={() => setIsSettingsOpen(true)}
          className="flex items-center gap-3 w-full hover:bg-surfaceElevated transition-colors rounded-2xl p-3 text-left text-gray-300"
        >
          <Settings size={18} />
          Settings
        </button>
